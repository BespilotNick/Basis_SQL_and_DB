...
/* Если я правильно понял, то лайки которые ПОЛУЧИЛ пользователь это likes.media_id,
 * тогда получается примерно так */


SELECT COUNT(l.media_id) AS 'Общее количество полученных лайков'
FROM likes l 
WHERE l.user_id IN (
	(SELECT id FROM users u WHERE u.id IN 
		(SELECT user_id FROM profiles p WHERE p.birthday > '2011-03-13')));
	
/*Тут у меня возникла проблема - почему-то при использовании в запросе и во вложенном запросе
 * знака "=", выдает ошибку (что-то вроде "подзапрос возвращает больше 1 строки").
 * Использовать "IN" я придумал медотом "научного тыка", но, кажется, работает */


-- Задание 2. Определить кто больше поставил лайков (всего): мужчины или женщины.
	
SELECT COUNT(l.user_id) AS 'Женщины',
(SELECT COUNT(l.user_id)
FROM likes l 
WHERE l.user_id IN 
	(SELECT id FROM users u WHERE u.id IN 
		(SELECT user_id FROM profiles p WHERE p.gender = 'm'))) AS 'Мужчины'
FROM likes l 
WHERE l.user_id IN 
	(SELECT id FROM users u WHERE u.id IN 
...
-- Задание 4. (по желанию)* Пусть задан некоторый пользователь. Из всех друзей этого пользователя найдите человека, который больше всех написал ему сообщений.

SELECT from_user_id AS 'id отправителя', 
COUNT(from_user_id) 'Количество сообщений' 
FROM messages m 
...
SELECT CONCAT(firstname, ' ', lastname) AS 'Пользователь, который больше всех написал сообщений' 
FROM users u
WHERE u.id =  
	(SELECT from_user_id
	FROM messages m 
